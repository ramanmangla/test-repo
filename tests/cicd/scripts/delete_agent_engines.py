...
def delete_single_agent_engine(engine, retry_count: int = 0) -> bool:
    ...
    try:
        ...
        if "contains child resources" in str(e):
            logger.warning(f"⚠️ Agent Engine {engine_name} has child resources, attempting force deletion...")
            try:
                # Force delete with child resources
                engine.delete(force=True)
                logger.info(f"✅ Force deleted Agent Engine with child resources: {engine_name}")
                return True
        ...
        if retry_count < MAX_RETRIES:
            logger.warning(f"⏱️ Rate limit hit for {engine_name}, waiting {RATE_LIMIT_DELAY} seconds before retry {retry_count + 1}/{MAX_RETRIES}...")
        ...
        if retry_count < MAX_RETRIES:
            logger.warning(f"⏱️ Error deleting {engine_name}, retrying in {RETRY_DELAY} seconds... (attempt {retry_count + 1}/{MAX_RETRIES})")
            time.sleep(RETRY_DELAY)
            return delete_single_agent_engine(engine, retry_count + 1)
        else:
            logger.error(f"❌ Failed to delete {engine_name} after {MAX_RETRIES} retries: {e}")
            return False


def delete_agent_engines_in_project(project_id: str, region: str = DEFAULT_REGION) -> tuple[int, int]:
    """
    ...
    try:
        ...
        logger.info(f"🎉 Deleted {deleted_count}/{len(engines)} Agent Engine service(s) in {project_id}")
...
def main():
    ...
    logger.info(f"📁 Projects processed: {len(project_ids) - len(failed_projects)}/{len(project_ids)}")
    
    if failed_projects:
        logger.warning(f"⚠️ Failed to process projects: {', '.join(failed_projects)}")
        sys.exit(1)
    elif total_found > total_deleted:
        logger.warning(f"⚠️ Some Agent Engine services could not be deleted ({total_found - total_deleted} failures)")
        sys.exit(1)
    else:
